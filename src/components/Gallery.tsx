  ...
  return (
    <section id="arena" className="py-32 bg-slate-900">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-12 md:grid-rows-2 gap-4 md:gap-6 h-auto md:h-[700px]">
          {images.map((img, idx) => (
            <div 
              key={idx} 
              className={`${img.span} relative group overflow-hidden rounded-[1.5rem] md:rounded-[2.5rem] bg-slate-800 shadow-2xl h-[220px] sm:h-[350px] md:h-auto hover:z-10 transition-all`}
            >
              <img 
                src={img.url} 
                alt={img.title}
              ...
              <div className="absolute inset-0 p-6 md:p-8 flex flex-col justify-end translate-y-2 md:translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                <span className="text-lime-400 text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] mb-1 md:mb-2">{img.category}</span>
                <div className="flex items-center justify-between gap-4">
                  <h4 className="text-xl md:text-3xl font-black italic text-white tracking-tighter uppercase leading-none">{img.title}</h4>
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity scale-75 group-hover:scale-100 duration-500 hidden md:flex">
